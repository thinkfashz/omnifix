...
export default function FirstAdminPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.setTimeout(() => {
        window.location.href = `/admin/login?setup=created&email=${encodeURIComponent(cleanEmail)}`;
      }, 700);
    } catch {
      setError('Error de red. Intenta nuevamente. También puedes pulsar el botón alternativo de abajo.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#020617] px-4 py-8 text-white">
      ...
      <section className="relative mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-lg flex-col justify-center">
        ...
        <form method="post" action="/api/admin/init-account" onSubmit={(event) => void submit(event)} className="rounded-[2rem] border border-blue-300/15 bg-white/[0.07] p-6 shadow-[0_28px_90px_rgba(0,0,0,.42)] backdrop-blur-2xl sm:p-8">
          ...
          <div className="mt-6 space-y-4">
            <Input name="initSecret" label="Clave de activación" type="password" value={setupKey} onChange={setSetupKey} placeholder="Clave de activación" />
            <Input name="name" label="Nombre" value={name} onChange={setName} placeholder="Admin Omnifix" />
            <Input name="email" label="Correo nuevo" type="email" value={email} onChange={setEmail} placeholder="adminnuevo@omnifix.cl" />
            <div>
              ...
              <div className="flex h-14 items-center rounded-2xl border border-blue-300/15 bg-white/[0.055] px-5 focus-within:border-blue-300/50">
                <input name="password" type={show ? 'text' : 'password'} value={pass} onChange={(e) => setPass(e.target.value)} placeholder="Mínimo 8 caracteres" className="h-full min-w-0 flex-1 bg-transparent text-sm font-semibold text-white outline-none placeholder:text-white/25" />
                <button type="button" onClick={() => setShow(!show)} className="ml-3 text-blue-100/70" aria-label="Mostrar u ocultar clave">{show ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}</button>
              </div>
            </div>
            <Input name="confirmPassword" label="Confirmar clave" type={show ? 'text' : 'password'} value={pass2} onChange={setPass2} placeholder="Repite la clave" />
            ...
            {message && <div className="rounded-2xl border border-emerald-400/25 bg-emerald-400/10 px-4 py-3 text-sm leading-6 text-emerald-200">{message}</div>}
            <button type="submit" disabled={loading} className="h-14 w-full rounded-full bg-blue-500 text-sm font-black uppercase tracking-[0.20em] text-white shadow-[0_18px_44px_rgba(37,99,235,.35)] transition hover:bg-white hover:text-blue-700 disabled:opacity-60">{loading ? 'Creando...' : 'Crear acceso'}</button>
            <p className="text-center text-[11px] leading-5 text-slate-400">Si el navegador no ejecuta JavaScript, este mismo botón funciona como formulario nativo y te redirige al login.</p>
...
function Input({ name, label, type = 'text', value, onChange, placeholder }: { name: string; label: string; type?: string; value: string; onChange: (value: string) => void; placeholder: string }) {
  return <label className="block"><span className="mb-2 block text-[10px] font-black uppercase tracking-[0.28em] text-blue-200/75">{label}</span><input name={name} type={type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="h-14 w-full rounded-2xl border border-blue-300/15 bg-white/[0.055] px-5 text-sm font-semibold text-white outline-none placeholder:text-white/25 transition focus:border-blue-300/50" /></label>;
}
