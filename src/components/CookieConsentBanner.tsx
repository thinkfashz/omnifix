...
const KEY = 'omnifix.cookie-consent.v2';
    ...
    <section className="fixed inset-x-3 bottom-3 z-[9998] mx-auto max-w-3xl rounded-[1.6rem] border border-blue-400/20 bg-slate-950/95 p-4 text-white shadow-[0_24px_80px_rgba(0,0,0,.45)] backdrop-blur-2xl md:p-5" aria-label="Consentimiento de privacidad y cookies">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-blue-300">Privacidad Omnifix Chile</p>
          <h2 className="mt-2 text-lg font-black">Cookies, datos de tienda y permisos</h2>
          <p className="mt-2 text-sm leading-6 text-slate-300">Usamos cookies necesarias para iniciar sesión, mantener el carrito, recordar preferencias y proteger la compra. Con tu autorización podemos usar analíticas y marketing. Omnifix puede tratar nombre, correo, teléfono, dirección, comuna, carrito, pedidos, soporte, dispositivo e interacciones para compra, despacho, seguridad, atención y mejora del servicio, conforme a la normativa chilena aplicable.</p>
        ...
        <div className="flex shrink-0 flex-col gap-2 md:w-56">
          <button onClick={() => save({ analytics: true, marketing: true })} className="rounded-full bg-blue-400 px-5 py-3 text-xs font-black uppercase tracking-[.16em] text-black">Aceptar todo</button>
      ...
      {settings && <div className="mt-4 grid gap-2 border-t border-white/10 pt-4 text-sm text-slate-300"><label className="flex items-center justify-between gap-3 rounded-2xl bg-white/[.04] p-3"><span><b className="block text-white">Analíticas</b><small>Medición de visitas, rendimiento, errores y uso de tienda.</small></span><input type="checkbox" checked={analytics} onChange={(e) => setAnalytics(e.target.checked)} /></label><label className="flex items-center justify-between gap-3 rounded-2xl bg-white/[.04] p-3"><span><b className="block text-white">Marketing</b><small>Campañas, ofertas y recomendaciones comerciales.</small></span><input type="checkbox" checked={marketing} onChange={(e) => setMarketing(e.target.checked)} /></label><button onClick={() => save({ analytics, marketing })} className="mt-2 rounded-full bg-white px-5 py-3 text-xs font-black uppercase tracking-[.16em] text-black">Guardar preferencias</button></div>}
    </section>
  );
}
