...
export default function OmnifixMobileBottomNav() {
  ...
  const results = useMemo(() => { const term = q.trim().toLowerCase(); if (!term) return list.slice(0, 8); return list.filter((p) => `${p.name} ${p.description || ''} ${category(p)}`.toLowerCase().includes(term)).slice(0, 12); }, [list, q]);
      ...
      {searchOpen && (
        <div className="fixed inset-0 z-[9300] bg-slate-950/72 backdrop-blur-xl md:hidden">
          <section className="absolute inset-x-3 bottom-24 max-h-[78dvh] overflow-hidden rounded-[2rem] border border-blue-300/20 bg-white text-slate-950 shadow-[0_24px_90px_rgba(0,0,0,.36)]">
            ...
            <div className="max-h-[calc(78dvh-78px)] space-y-3 overflow-y-auto p-4 pb-6">{results.map((p) => <article key={p.id} className="flex gap-3 rounded-[1.25rem] bg-slate-50 p-2 ring-1 ring-slate-200"><img src={image(p)} alt={p.name} className="h-20 w-20 rounded-2xl object-cover" /><div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-[.18em] text-blue-500">{category(p)}</p><h3 className="line-clamp-2 text-sm font-black leading-tight">{p.name}</h3><div className="mt-2 flex items-center justify-between gap-2"><b className="text-sm text-blue-700">{clp(price(p))}</b><button onClick={() => addToCart(toCart(p))} className="rounded-full bg-blue-600 px-3 py-2 text-[10px] font-black uppercase text-white">Agregar</button></div></div></article>)}{results.length === 0 && <p className="rounded-2xl bg-slate-50 p-6 text-center text-sm font-bold text-slate-500">No encontré productos con esa búsqueda.</p>}</div>
          </section>
        </div>
      )}
