  ...
  return (
    <section className="py-10">
      <div className="text-center mb-14">
        <p className="text-blue-400/80 text-xs tracking-[0.4em] uppercase font-semibold mb-4 hero-text-fade" style={{ animationDelay: '0s', animationFillMode: 'both' }}>Catálogo de Soluciones</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {visibleProducts.map((prod, i) => {
            ...
            <article
              key={prod.id}
              className="card-3d glass-card rounded-4xl overflow-hidden group relative cursor-pointer focus-within:ring-2 focus-within:ring-blue-400/60 slide-up"
            ...
            >
              <div className="relative h-48 bg-white/5 overflow-hidden">
                {prod.img ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-zinc-950">
                    <span className="font-playfair text-4xl font-bold text-blue-400/20 tracking-widest">FBK</span>
                  </div>
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-black/75 to-transparent" />
                {prod.featured ? (
                  <span className="absolute top-3 left-3 px-3 py-1 rounded-full bg-blue-400 text-black text-[10px] font-bold tracking-wider">
                    Destacado
                  </span>
                ) : null}
                {hasDiscount ? (
              ...
              <div className="p-6 flex flex-col gap-4 min-h-[17rem]">
                <div>
                  <p className="text-blue-400/60 text-xs tracking-[0.25em] uppercase mb-1">{prod.delivery}</p>
                ...
                <div className="flex items-end justify-between mt-auto pt-4 border-t border-white/5 gap-4">
                  <div>
                    {hasDiscount ? (
                      <div className="flex items-baseline gap-2">
                        <p className="text-blue-400 font-bold text-2xl font-playfair">{CLP(finalPrice)}</p>
                        <p className="text-zinc-500 text-xs line-through">{CLP(prod.price)}</p>
                      </div>
                    ) : (
                      <p className="text-blue-400 font-bold text-2xl font-playfair">{CLP(prod.price)}</p>
                    )}
                  ...
                  <AnimatedButton
                    ...
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-blue-400/10 border border-blue-400/30 text-blue-400 text-xs font-semibold tracking-wider hover:bg-blue-400 hover:text-black transition-all duration-300"
                  >
                    Ver producto
      ...
      <div className="mt-12 flex flex-wrap items-center justify-center gap-4">
        <Link
          href={primaryCtaHref}
          className="inline-flex items-center gap-3 rounded-full bg-blue-400 px-8 py-4 text-[11px] font-black uppercase tracking-[0.22em] text-black transition hover:bg-white"
        >
          {primaryCtaLabel}
