...
const navSections: Section[] = [
  ...
  { title: 'IA e integraciones', links: [
    ...
    { href: '/admin/integraciones', label: 'Integraciones', description: 'Shopify, APIs, webhooks y servicios', icon: Link2, highlight: true },
...
function AdminClock() { const [now, setNow] = useState<Date | null>(null); useEffect(() => { setNow(new Date()); const id = setInterval(() => setNow(new Date()), 1000); return () => clearInterval(id); }, []); return <span className="flex items-center gap-1.5 text-[10px] font-semibold tabular-nums text-slate-400"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-blue-400" />{now?.toLocaleTimeString('es-CL', { hour12: false }) || '--:--:--'}</span>; }
function AvatarCircle({ photo }: { photo?: string | null }) { return <span className="relative flex h-9 w-9 shrink-0 overflow-hidden rounded-full border-2 border-blue-300/40 bg-black/60 shadow-lg">{photo ? <img src={photo} alt="Foto de perfil" className="h-full w-full object-cover" /> : <User className="m-auto h-5 w-5 text-blue-300/70" />}</span>; }
...
function SidebarContent({ pathname, onNavigate, onLogout, onClose, role, profilePhoto }: { pathname: string; onNavigate?: () => void; onLogout: () => void; onClose?: () => void; role: string | null; profilePhoto?: string | null }) {
  ...
  const visible = useMemo(() => { const term = search.trim().toLowerCase(); return allowed.map((s) => ({ ...s, links: term ? s.links.filter((l) => `${l.label} ${l.description} ${s.title}`.toLowerCase().includes(term)) : s.links })).filter((s) => s.links.length); }, [allowed, search]);
  const featured = allowed.flatMap((s) => s.links).filter((l) => l.highlight).slice(0, 6);
  return <div className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-[2rem] border border-blue-300/14 bg-slate-950/88 shadow-[0_26px_80px_rgba(15,23,42,.42)] backdrop-blur-2xl"><div className="shrink-0 border-b border-blue-300/10 p-4"><div className="flex items-center gap-3"><BrandMark size="lg" animated /><div className="min-w-0 flex-1"><BrandWordmark tagline="Admin · Control room" /><div className="mt-2 flex items-center gap-2"><span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2 py-0.5 text-[8.5px] font-black uppercase tracking-[0.18em] text-emerald-300">{role === 'superadmin' ? 'Superadmin' : role === 'viewer' ? 'Demo' : 'Admin'}</span><AdminClock /></div></div>{onClose && <button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-2xl border border-white/12 bg-black/35 text-white/70"><X className="h-5 w-5" /></button>}</div><label className="relative mt-4 block"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/35" /><input type="search" placeholder="Buscar productos, pedidos, IA..." value={search} onChange={(e) => setSearch(e.target.value)} className="h-11 w-full rounded-2xl border border-blue-300/12 bg-black/30 pl-10 pr-3 text-sm text-white outline-none placeholder:text-white/30 focus:border-blue-300/45" /></label></div><div className="min-h-0 flex-1 overflow-y-auto p-3 scrollbar-hide">{!search.trim() && <div className="mb-3 rounded-[1.35rem] border border-blue-300/20 bg-blue-400/[0.055] p-2.5"><p className="px-1 pb-2 text-[9px] font-black uppercase tracking-[0.28em] text-blue-200/70">Accesos rápidos</p><div className="grid grid-cols-2 gap-2">{featured.map((l) => <Link key={l.href} href={l.href} onClick={onNavigate} className="flex min-w-0 items-center gap-2 rounded-2xl border border-white/10 bg-black/25 px-2.5 py-2 hover:border-blue-300/35"><l.icon className="h-3.5 w-3.5 shrink-0 text-blue-300" /><span className="truncate text-[10px] font-black text-white/82">{l.label}</span></Link>)}</div></div>}{visible.length === 0 ? <div className="rounded-[1.5rem] border border-white/10 bg-black/25 px-4 py-8 text-center text-sm text-white">Sin módulos encontrados</div> : <div className="space-y-3">{visible.map((s) => <nav key={s.title} className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-black/22"><p className="px-4 pt-4 text-[10px] font-black uppercase tracking-[0.28em] text-blue-200/70">{s.title}</p><div className="space-y-1.5 p-2">{s.links.map((l) => <NavItem key={`${s.title}-${l.href}`} {...l} active={isActivePath(pathname, l.href)} onNavigate={onNavigate} />)}</div></nav>)}</div>}</div><button type="button" onClick={onLogout} className="m-3 flex h-12 items-center justify-center gap-2 rounded-2xl border border-white/10 bg-black/30 text-xs font-black uppercase tracking-[0.18em] text-slate-300 hover:text-rose-300"><LogOut className="h-4 w-4" /> Salir</button></div>;
}
