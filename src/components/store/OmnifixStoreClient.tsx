...
type Product = { id: string; name: string; price: number; description?: string; image_url?: string; img?: string; category?: string; category_id?: string; category_name?: string; stock?: number | string; discount_percentage?: number; discountPercentage?: number; shopifyVariantId?: string; shopifyProductId?: string; shopifyHandle?: string; source?: string };
...
export default function OmnifixStoreClient() {
  ...
  const list = useMemo<Product[]>(() => { const map = new Map<string, Product>(); [...(products as Product[]), ...(OMNIFIX_EXTRA_PRODUCTS as Product[])].forEach((product) => map.set(product.id, product)); return Array.from(map.values()); }, [products]);
  ...
  return <main className="min-h-screen bg-[#020617] pb-32 text-white"><div className="fixed inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(37,99,235,.32),transparent_34rem),radial-gradient(circle_at_90%_70%,rgba(34,211,238,.16),transparent_28rem)]" />
    <section className="relative mx-auto max-w-7xl px-4 py-6 md:px-8">
      <header className="relative flex items-center justify-center md:justify-between"><Link href="/" className="flex items-center justify-center text-center md:justify-start md:text-left"><FabrickLogo className="pointer-events-none" /></Link><button onClick={openCart} className="relative hidden h-12 w-12 place-items-center rounded-2xl bg-blue-400 text-black md:grid"><ShoppingBag className="h-5 w-5" />{totalItems > 0 && <span className="absolute -right-1 -top-1 rounded-full bg-cyan-200 px-1 text-[10px] font-black">{totalItems}</span>}</button></header>
      <div className="mt-8 rounded-[2rem] border border-blue-400/15 bg-white/[.06] p-6 backdrop-blur-2xl md:p-10"><p className="text-[10px] font-black uppercase tracking-[.30em] text-cyan-200/70">Tienda Omnifix</p><h1 className="mt-4 text-5xl font-black leading-[.9] tracking-[-.07em] md:text-7xl">Productos tech, sin vueltas.</h1><p className="mt-5 max-w-xl text-sm leading-7 text-slate-300">Toca un producto para ver su ficha completa o agrégalo directo al carrito.</p></div>
      ...
      <div className="mt-5 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{filtered.map((p) => { const ok = p.shopifyVariantId?.startsWith('gid://shopify/ProductVariant/'); const needsShopifyVariant = p.source === 'shopify' || Boolean(p.shopifyProductId || p.shopifyHandle); return <article key={p.id} className="overflow-hidden rounded-[1.8rem] border border-blue-400/15 bg-white/[.055] backdrop-blur-xl"><Link href={href(p)} className="block"><div className="h-56 bg-white/5"><img src={image(p)} alt={p.name} className="h-full w-full object-cover" /></div><div className="p-5 pb-0"><p className="text-[10px] font-black uppercase tracking-[.22em] text-blue-300">{category(p)}</p><h2 className="mt-2 line-clamp-2 text-xl font-black">{p.name}</h2><p className="mt-2 line-clamp-2 text-sm text-slate-400">{p.description || 'Producto seleccionado por Omnifix.'}</p></div></Link><div className="p-5"><div className="flex items-center justify-between gap-3"><b className="text-xl font-black text-blue-300">{clp(final(p))}</b><button onClick={() => addToCart(toCart(p))} disabled={needsShopifyVariant && !ok} className="rounded-full bg-blue-400 px-4 py-3 text-xs font-black uppercase text-black disabled:opacity-45">Agregar</button></div>{needsShopifyVariant && !ok && <p className="mt-3 text-xs text-blue-200/80">Producto sin variante Shopify válida.</p>}</div></article>; })}</div>
